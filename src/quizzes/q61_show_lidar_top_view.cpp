...
void showLidarTopview()
{
    ...
    for (auto it = lidarPoints.begin(); it != lidarPoints.end(); ++it)
    {
        ...
        // remove points over ground
        if (it->z > -1.40)
        {
            // color in points, red/green intensity based distance in x-direction
            float i = it->x;
            float iMax = worldSize.height;
            int r = min(255, (int)(255 * abs((i-iMax)/iMax)));
            int g = min(255, (int)(255 * (1-abs((i-iMax)/iMax))));
